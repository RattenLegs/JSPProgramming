...
<br>
2. 웹 브라우저 캐시 제어를 위한 응답 헤더 입력<br>
캐시: 동일한 데이터를 중복해서 로딩하지 않도록 할 때 사용<br>
브라우저는 첫번째 요청 시 로컬 PC의 임시 보관소의 캐시에 저장<br>
이후 동일한 URL에 대한 요청이 있으면 WAS에 접근하지 않고 로컬 PC에 저장한 응답 결과를 사용한다.<br>
캐시에 보관된 데이터를 사용하는 경우 WAS에 접근하지 않기 때문에 이미지, 정적인 HTML 등은 캐시에 보관함으로 응답 속도가 향상시킬 수 있다.<br>
하지만 게시판처럼 내용이 자주 변경되는 사이트는 웹 브라우저 캐시가 적용되면 사용자는 변경된 내용을 확인할 수 없다.<br>

<br>
response 기본 객체가 제공하는 헤더 추가 메서드<br>
